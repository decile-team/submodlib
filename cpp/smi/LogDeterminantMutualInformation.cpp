...
double LogDeterminantMutualInformation::marginalGainWithMemoization(std::unordered_set<ll> const &X, ll item) {
    double gain = 0;
...
std::vector<std::pair<ll, double>> LogDeterminantMutualInformation::maximize(std::string optimizer,ll budget, bool stopIfZeroGain=false, bool stopIfNegativeGain=false, float epsilon = 0.1, bool verbose=false) {
