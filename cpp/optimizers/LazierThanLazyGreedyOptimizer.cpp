...
bool LazierThanLazyGreedyOptimizer::equals(double val1, double val2, double eps) {
...
bool printSortedSet(std::set<std::pair<float, ll>> &sortedSet) {
	std::cout << "[";
  for (auto rev_it = sortedSet.rbegin(); rev_it != sortedSet.rend(); rev_it++) {
		std::cout << "("<< (*rev_it).first << ", " << (*rev_it).second << "), ";
	}
	std::cout << "]\n";
}

std::vector<std::pair<ll, float>> LazierThanLazyGreedyOptimizer::maximize(SetFunction &f_obj, ll budget, bool stopIfZeroGain=false, bool stopIfNegativeGain=false, float epsilon = 0.1, bool verbose=false) {
  ...
  int i = 0;
	while (rem_budget > 0) {
		...
		if(verbose) std::cout << "Now running lazy greedy on the random set\n";
	  ll best_id;
		float best_val;
		bool done = false;
		while (!done) {
			...
			//printSortedSet(sortedGains);
			for (auto rev_it = sortedGains.rbegin(); rev_it != sortedGains.rend(); rev_it++) {
				//std::cout << "Checking " << (*rev_it).second << "...\n";
				if (randomSet.find((*rev_it).second) != randomSet.end()) {
					//std::cout << "...present in random set....\n";
					candidate_id = (*rev_it).second;
					candidate_val = (*rev_it).first;
					newCandidateBound =
										f_obj.marginalGainWithMemoization(greedySet,
																											candidate_id);
					//std::cout << "Updated gain as per updated greedy set = " << newCandidateBound << "\n";
					auto nextElem = std::next(rev_it, 1);
					if (newCandidateBound > (*nextElem).first) {
						//std::cout << "..better than next best upper bound, selecting...\n";
						best_id = candidate_id;
						best_val = newCandidateBound;
						done = true;
						break;
					} else {
						//std::cout << "... NOT better than next best upper bound, updating...\n";
						done = false;
						break;
					}
				} else {
