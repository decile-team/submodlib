...
class ProbabilisticSetCoverFunction(SetFunction):
	...
	def evaluate(self, X):
		"""Computes the Probabilistic Set Cover score of a set

		Parameters
		----------
		X : set
			The set whose Probabilistic Set Cover score needs to be computed
		
		Returns
		-------
		float
			The Probabilistic Set Cover function evaluation on the given set
	...
	def maximize(self, budget, optimizer='NaiveGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, epsilon = 0.1, verbose=False):
		"""Find the optimal subset with maximum Probabilistic Set Cover score for a given budget
	...
	def marginalGain(self, X, element):
		"""Find the marginal gain in Probabilistic Set Cover score when a single item (element) is added to a set (X)
	...
	def marginalGainWithMemoization(self, X, element):
		"""Efficiently find the marginal gain in Probabilistic Set Cover score when a single item (element) is added to a set (X) assuming that memoized statistics for it are already computed
	...
	def evaluateWithMemoization(self, X):
		"""Efficiently compute the Probabilistic Set Cover score of a set assuming that memoized statistics for it are already computed
	...
	def getEffectiveGroundSet(self):
		"""Get the effective ground set of this Probabilistic Set Cover object. This is equal to the ground set when instantiated with partial=False and is equal to the ground_sub when instantiated with partial=True
