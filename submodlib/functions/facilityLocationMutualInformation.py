...
class FacilityLocationMutualInformationFunction(SetFunction):
	...
	def evaluate(self, X):
		"""Computes the FacilityLocationMutualInformation score of a set

		Parameters
		----------
		X : set
			The set whose FacilityLocationMutualInformation score needs to be computed
		
		Returns
		-------
		float
			The FacilityLocationMutualInformation function evaluation on the given set
	...
	def maximize(self, budget, optimizer='NaiveGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, epsilon = 0.1, verbose=False):
		"""Find the optimal subset with maximum FacilityLocationMutualInformation score for a given budget
	...
	def marginalGain(self, X, element):
		"""Find the marginal gain in FacilityLocationMutualInformation score when a single item (element) is added to a set (X)
	...
	def marginalGainWithMemoization(self, X, element):
		"""Efficiently find the marginal gain in FacilityLocationMutualInformation score when a single item (element) is added to a set (X) assuming that memoized statistics for it are already computed
	...
	def evaluateWithMemoization(self, X):
		"""Efficiently compute the FacilityLocationMutualInformation score of a set assuming that memoized statistics for it are already computed
	...
	def getEffectiveGroundSet(self):
		"""Get the effective ground set of this FacilityLocationMutualInformation object. This is equal to the ground set when instantiated with partial=False and is equal to the ground_sub when instantiated with partial=True
