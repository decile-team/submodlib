...
class LogDeterminantConditionalGainFunction(SetFunction):
	...
	def __init__(self, n, num_privates, lambdaVal, image_sijs=None, private_sijs=None, private_private_sijs=None, imageData=None, privateData=None, metric="cosine", privacyHardness=1):
		self.n = n
		...
		if (type(self.image_sijs) != type(None)) and (type(self.private_sijs) != type(None)) and (type(self.private_private_sijs) != type(None)): # User has provided all three kernels
			...
			if (type(self.imageData) != type(None)) or (type(self.privateData) != type(None)):
				print("WARNING: similarity kernels found. Provided image and private data matrices will be ignored.")
		...
		self.cpp_obj = LogDeterminantConditionalGain(self.n, self.num_privates, self.cpp_image_sijs, self.cpp_private_sijs, self.cpp_private_private_sijs, self.lambdaVal, self.privacyHardness)
		self.effective_ground = set(range(n))

	def evaluate(self, X):
		"""Computes the LogDeterminantConditionalGain score of a set

		Parameters
		----------
		X : set
			The set whose LogDeterminantConditionalGain score needs to be computed
		
		Returns
		-------
		float
			The LogDeterminantConditionalGain function evaluation on the given set
	...
	def maximize(self, budget, optimizer='NaiveGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, epsilon = 0.1, verbose=False):
		"""Find the optimal subset with maximum LogDeterminantConditionalGain score for a given budget
	...
	def marginalGain(self, X, element):
		"""Find the marginal gain in LogDeterminantConditionalGain score when a single item (element) is added to a set (X)
	...
	def marginalGainWithMemoization(self, X, element):
		"""Efficiently find the marginal gain in LogDeterminantConditionalGain score when a single item (element) is added to a set (X) assuming that memoized statistics for it are already computed
	...
	def evaluateWithMemoization(self, X):
		"""Efficiently compute the LogDeterminantConditionalGain score of a set assuming that memoized statistics for it are already computed
	...
	def getEffectiveGroundSet(self):
		"""Get the effective ground set of this LogDeterminantConditionalGain object. This is equal to the ground set when instantiated with partial=False and is equal to the ground_sub when instantiated with partial=True
