...
class DisparitySumFunction(SetFunction):
	...
	def evaluate(self, X):
		"""Computes the Disparity-Sum score of a set

		Parameters
		----------
		X : set
			The set whose Disparity-Sum score needs to be computed
		
		Returns
		-------
		float
			The Disparity-Sum function evaluation on the given set
	...
	def maximize(self, budget, optimizer='NaiveGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, epsilon = 0.1, verbose=False):
		"""Find the optimal subset with maximum Disparity-Sum score for a given budget
	...
	def marginalGain(self, X, element):
		"""Find the marginal gain in Disparity-Sum score when a single item (element) is added to a set (X)
	...
	def marginalGainWithMemoization(self, X, element):
		"""Efficiently find the marginal gain in Disparity-Sum score when a single item (element) is added to a set (X) assuming that memoized statistics for it are already computed
	...
	def evaluateWithMemoization(self, X):
		"""Efficiently compute the Disparity-Sum score of a set assuming that memoized statistics for it are already computed
	...
	def getEffectiveGroundSet(self):
		"""Get the effective ground set of this Disparity-Sum object. This is equal to the ground set when instantiated with partial=False and is equal to the ground_sub when instantiated with partial=True
